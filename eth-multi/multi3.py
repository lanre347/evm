...
def send_percentage_balance(private_key, percentage, wallet_number=None, custom_to_address=None):
    sender = web3.eth.account.from_key(private_key).address
...
def automate_daily_transfers():
    ...
    if PREVIOUS_CHOICE == "4":
        ...
        for wallet_number, pk in enumerate(private_keys, start=1):
            send_percentage_balance(pk, percentage, wallet_number, custom_to_address)
        return
